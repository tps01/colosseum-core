...
def _run_suite_expect_exit(suite_path, bench_sim, isolated_cwd, code: int) -> None:
    with pytest.raises(SystemExit) as exc:
        run_suite(suite_path, bench_sim)
    assert exc.value.code == code


def test_happy_suite_single_output_db_and_summary(fixtures_dir, bench_sim, isolated_cwd) -> None:
...
def test_setup_failure_skips_tests_but_runs_teardown(fixtures_dir, bench_sim, isolated_cwd) -> None:
...
def test_teardown_failure_fails_run_even_when_tests_pass(fixtures_dir, bench_sim, isolated_cwd) -> None:
