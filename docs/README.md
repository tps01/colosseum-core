...
The ADR, FFO, and DDD documents remain design history and rationale. They are useful when changing behavior, but some wave-planning language in those documents predates the implemented MVP. When there is a conflict, prefer [mvp/scope.md](mvp/scope.md), the user guides, tests, the current code, and the **Bench configuration reference** generated by `python scripts/docgen/build_all.py`.

Removed pre-MVP files are listed in [archive/MANIFEST.md](archive/MANIFEST.md); recover them from git tag `doc-snapshot-pre-archive` (see [archive/README.md](archive/README.md)).
